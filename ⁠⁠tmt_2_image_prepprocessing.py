...
INPUT_DIR  = BASE_DIR / "dataset_augmented"
OUTPUT_DIR = BASE_DIR / "dataset_preprocessed"
...
# ─── Step 6: Edge-Preserving Smoothing (PIL bilateral approximation) ──────
def apply_edge_preserving_smooth(img):
    blurred = img.filter(ImageFilter.GaussianBlur(radius=1))
    arr_orig   = np.array(img).astype(np.float32)
    arr_blur   = np.array(blurred).astype(np.float32)
    diff       = arr_orig - arr_blur         
    edge_mask  = np.abs(diff).max(axis=2, keepdims=True) / 255.0  
    smoothed   = arr_blur * (1 - edge_mask) + arr_orig * edge_mask  
    return Image.fromarray(smoothed.clip(0, 255).astype(np.uint8))
...
def preprocess(img):
    ...
    img = apply_edge_preserving_smooth(img)
...
def process_class(cls):
    ...
    processed  = 0
    skipped    = 0
    used_names = set()   # track output names to handle duplicate stems

    for i, p in enumerate(src_paths, 1):
        try:
            img = preprocess(Image.open(p))

            # resolve name collision: if stem already used, append a counter
            out_stem = p.stem
